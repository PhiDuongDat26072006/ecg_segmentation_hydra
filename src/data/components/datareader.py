...
def load_ludb_tensors(ludb_files, leads=None):
    ...
    # finalize arrays
    all_waves = np.array(all_waves)   # (M * 12, 5000)
    all_p = np.array(all_p)           # (M * 12, 5000)
    all_qrs = np.array(all_qrs)       # (M * 12, 5000)
    all_t = np.array(all_t)           # (M * 12, 5000)
    all_none = np.array(all_none)     # (M * 12, 5000)
    all_seg_target = np.stack(arrays=(all_p,all_qrs,all_t,all_none), axis=1)  # (M * 12, 4, 5000)
    all_cls_target = np.array(all_cls_target)                                 # (M * 12, )

    # convert to torch tensors
    X_torch = torch.tensor(all_waves, dtype=torch.float32)
    X_torch = X_torch.unsqueeze(dim=1) # add channel dimension # (M * 12, 1, 5000)
